...
namespace IMLD.MixedRealityAnalysis.Views
{
    ...
    public class SimpleVisualizationSettingsView : AbstractSettingsView
    {
        ...
        private const float OffsetY = 0.16f;
        ...
        public override void ApplyChanges()
        {
            ...
            var config = new VisProperties(
                settings.VisId, settings.VisType, 
                settings.AnchorId, dataSets, new List<int>(Services.StudyManager().CurrentStudyConditions), new List<int>(Services.StudyManager().CurrentStudySessions));
        ...
        public override void Init(IConfigurableVisualization vis, bool showStaticObjects = true, bool showSpeedSettings = false)
        {
            ...
            if (SettingsPrefab && Services.DataManager() != null)
            {
                foreach (var dataSet in Services.DataManager().DataSets.Values)
                {
                    ...
                    {
                        var settingsViewObject = GameObject.Instantiate<SettingsViewObject>(SettingsPrefab, this.transform); // each row 
            ...
            if (cameraTransform != null)
            {
                ...
                ////transform.LookAt(cameraTransform.position, Vector3.up); cameraTransform.position + 
