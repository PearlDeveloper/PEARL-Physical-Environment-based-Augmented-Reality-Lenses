...
namespace SampleQRCodes
{
    ...
    public class QRCodesManager : Singleton<QRCodesManager>
    {
        ...
        public bool bStartQRTracking = false;
        public bool bStopQRTracking = false;
        ...
        {
            Debug.Log("Going through StartQRTracking");
            Debug.Log("qrTracker: " + qrTracker);
            Debug.Log("IsTrackerRunning: " + IsTrackerRunning);
            if(qrTracker == null)
                SetupQRTracking();
        ...
        {
            Debug.Log("Going through StopQRTracking!");
            Debug.Log("qrTracker: " + qrTracker);
            Debug.Log("IsTrackerRunning: " + IsTrackerRunning);
            if (IsTrackerRunning)
            {
            ...
            Debug.Log("QRTracking Stopped!");
        }

        public void restartQRTracking()
        {
            StopQRTracking();
            StartQRTracking();
        }
        ...
        private void Update()
        {
            if (qrTracker == null && capabilityInitialized && IsSupported) // We do not check if IsSupported is true or not 
            {
            ...
            if (bStopQRTracking)
            {
                bStopQRTracking = false;
                StopQRTracking();
            }

            if (bStartQRTracking)
            {
                bStartQRTracking = false;
                StartQRTracking();
            }
        }
    }
